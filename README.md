...
> plan-authoring state machine, two pipeline descriptors, tests, and agent
> guidance are present. The `run`, `resume`, and `status` workflows are not wired
> yet and deliberately return a non-zero exit code.
