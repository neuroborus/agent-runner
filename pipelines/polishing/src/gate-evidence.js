...
export function candidateGatePassed(
  ...
  const policy = polishingPolicy(state.settings);
  return (
...
export function terminalConfirmationGatePassed(
  state,
  fingerprint = state.finalizedFingerprint,
) {
  if (
    ...
    return false;
  if (
    polishingPolicy(state.settings).terminalConfirmer === "worker" &&
  ...
  const findingsAccepted =
    state.reviewResult.status === "APPROVED" ||
    (state.reviewResult.status === "FINDINGS" &&
      state.previousFindings.length > 0 &&
      state.previousFindings.every(({ id }) =>
        overridden(state.findingOverrides, id, fingerprint),
      ));
  if (!findingsAccepted) return false;
...
export function handoffGatePassed(
  state,
  fingerprint = state.finalizedFingerprint,
) {
  return (
...
function clearedConfirmationGate() {
  return {
