...
function migrateVersionOneState(state) {
  ...
  return migratePlanExecutionStateV12({ pipelineState: versionTwelve });
}
...
test("migrates version-3 execution state with no consumed bootstrap corrections", () => {
  ...
  assert.doesNotThrow(() => normalizePipelineState(migrated));
  assert.equal(planExecutionPipeline.stateVersion, 13);
});
...
test("selects Worker-only lazy mode and migrates version 11 to independent", () => {
  ...
  assert.equal(planExecutionPipeline.settings.mode.validate("automatic"), false);
...
test("normalizes strict lazy convergence results", () => {
  ...
  assert.deepEqual(normalizeCheckAndFixResult(checkAndFix("REFINALIZE")), {
    status: "REFINALIZE",
    summary: "The finalization evidence must be regenerated.",
  });
...
test("version 11 migration preserves terminal commits and effects", async (t) => {
  ...
  assert.deepEqual(migrated.completedCommits, completed.pipelineState.completedCommits);
  assert.equal(migrated.pendingCommit, null);
  ...
  assert.deepEqual(current.completedCommits, completed.pipelineState.completedCommits);
  assert.deepEqual(current.finalizationResult, completed.pipelineState.finalizationResult);
  ...
  const fixture = await createFixture(t, {
    worker: [
      clarificationReady(),
      {
        ...bootstrapReady("Worker"),
        requiredChecks: [
          { ...REQUIRED_CHECKS[0], unexpected: "rejected" },
        ],
      },
...
test("batches every staging-dependent finalization command into one correction", async (t) => {
  ...
  for (const rejectedCommand of rejectedCommands) {
    assert.doesNotMatch(correctionCall.prompt, new RegExp(rejectedCommand, "u"));
  }
...
test("reconstructs finalization correction before and during interruption", async (t) => {
  await t.test("before correction", async (t) => {
    ...
    const correctionCall = fixture.calls.worker.find(
      ({ prompt }) => prompt.includes("bounded read-only correction"),
    );
...
test("scopes finalization correction attempts to the current commit step", async (t) => {
  ...
  const corrections = fixture.transitions
    ...
    .map(({ patch }) => patch.pipelineState.finalizationCorrections.at(-1).step);
...
test("automatically corrects invalid lazy check output in a fresh session", async (t) => {
  ...
  const correctionState = fixture.transitions.find(
    ({ patch }) => patch?.pipelineState?.pendingLazyCorrection !== null &&
...
test("re-finalizes an invalid content-changing lazy check before correction", async (t) => {
  let changed = false;
  const fixture = await createFixture(t, {
    mode: "lazy",
    worker: [clarificationReady(), bootstrapReady("Worker")],
  ...
  assert.ok(
    fixture.transitions.some(
      ({ patch }) =>
        patch?.pipelineState?.workflowState === "FINALIZE" &&
        patch.pipelineState.pendingLazyCorrection?.phase === "CHECK_AND_FIX",
    ),
  );
  assert.ok(
    fixture.transitions.some(
...
test("does not recount an invalid lazy mutation after finalization restores its fingerprint", async (t) => {
  let changed = false;
  const fixture = await createFixture(t, {
    mode: "lazy",
    worker: [clarificationReady(), bootstrapReady("Worker")],
...
test("pauses and explicitly resumes an exhausted lazy correction without recounting", async (t) => {
  ...
  assert.doesNotMatch(JSON.stringify(projected), /DO_NOT_PERSIST|private output/u);
...
test("reconstructs an interrupted pending lazy correction without recounting", async (t) => {
  ...
  assert.equal(fixture.currentRun.pipelineState.pendingLazyCorrection.attempt, 1);
...
test("re-finalizes a content-changing lazy check before clean confirmation", async (t) => {
  let changed = false;
  const fixture = await createFixture(t, {
    mode: "lazy",
    worker: [clarificationReady(), bootstrapReady("Worker")],
  ...
  const phases = fixture.transitions
    ...
    .filter(Boolean);
  const changedCheck = phases.indexOf("FINALIZE", phases.indexOf("CHECK_AND_FIX"));
  const confirmation = phases.indexOf("CLEAN_CONFIRM", changedCheck);
  assert.ok(changedCheck >= 0);
  assert.ok(confirmation > changedCheck);
  assert.equal(fixture.calls.reviewer.length, 0);
  assert.equal(fixture.calls.arbiter.length, 0);
});
...
test("re-finalizes corrected evidence without requiring a content change", async (t) => {
    ...
    findings: [
      {
        ...
        reason: "The reported validation fingerprint does not cover the complete inventory.",
  ...
  assert.equal(finalizationCalls.length, 2);
  assert.match(
    finalizationCalls[1].prompt,
    /finalization inventory omitted an established test file/u,
  );
  assert.equal(fixture.calls.reviewer.length, 0);
  assert.equal(fixture.calls.arbiter.length, 0);
});
...
test("pauses repeated invalid re-finalization without a confirmation finding", async (t) => {
  ...
  assert.equal(paused.pause.resumeState, "CHECK_AND_FIX");
  assert.deepEqual(
    paused.pipelineState.pendingLazyCorrection.diagnostics,
    [
      {
        role: "worker",
        phase: "check-and-fix",
        ...
        field: "status",
        constraint: "requires-confirmation-finding",
      },
    ],
  );
});
  ...
  const fixture = await createFixture(t, {
    ...
    async onRoleRun(role, request) {
      if (
        ...
        mutated = true;
        await writeFile(join(request.cwd, "invalid-confirmation.txt"), "changed\n");
      }
    },
  });
...
test("resumes a reconciled lazy check/fix without replay or recounting", async (t) => {
  const processLoss = new Error("Process stopped after lazy fix reconciliation.");
  let changed = false;
  const fixture = await createFixture(t, {
    mode: "lazy",
    sourceSession: SOURCE_SESSION,
    ...
    async onRoleRun(role, request) {
      if (
        ...
        changed = true;
        await writeFile(join(request.cwd, "reconciled-lazy-fix.txt"), "fixed\n");
      }
    },
  });
...
test("bounds repeated lazy confirmation findings without arbitration", async (t) => {
  ...
  assert.equal(
    planExecutionPipeline
      .projections.pause(fixture.currentRun)
      .nextActions.some(
        ({ action }) => action?.type === "override-finding",
      ),
    false,
...
test("applies additional lazy fix rounds without resetting prior progress", async (t) => {
  ...
  const result = await fixture.run(
    {},
    { type: "extra-fix-rounds", amount: 3 },
  );
...
test("bounds repeated content-changing lazy checks", async (t) => {
  ...
  const result = await fixture.run(
    {},
    { type: "extra-fix-rounds", amount: 2 },
  );
