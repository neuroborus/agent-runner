...
const pipelineMode = z
  ...
  .describe(
    "independent is the default and recommended mode with genuinely independent review and higher context/token use. lazy is an explicit lower-consumption choice without independent review; never select it automatically. combined adds primary convergence before independent review and is currently supported only by plan-authoring.",
  );
...
export function createMcpServer(options = {}) {
  ...
  server.registerTool(
    ...
    {
      description:
        "Start a durable pipeline. independent is default and recommended for genuinely independent semantic review but uses more provider context and tokens; lazy is opt-in for lower consumption and has no independent review, so never select it automatically. combined adds primary convergence before independent review and is available only for plan-authoring; unsupported pipeline selections reject. Leave sourceSession unset unless the user deliberately selects a compatible current session after being offered a fresh start. independent and combined fork its complete context into primary and review roles; lazy forks it once into the primary role. Recommend fresh for a long, multi-topic, or uncertain session. Include its known trusted profile, use only current inheritance when unknown, keep native IDs opaque, and never inspect private storage or infer an ID or alias.",
